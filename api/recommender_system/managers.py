...
class RecommenderSystemManger(models.Manager):
    def calculate_cosine_similarity(self, user_ratings):
        ...
        for rating in user_ratings:
            if rating.user_id not in user_rating_dict:
                user_rating_dict[rating.user_id] = {}
            user_rating_dict[rating.user_id][rating.game_element_id] = rating.rating
        ...
        for i, user1 in enumerate(user_ids):
            for j, user2 in enumerate(user_ids):
                if i == j:
                    ...
                    user1_ratings = user_rating_dict.get(user1, {})
                    user2_ratings = user_rating_dict.get(user2, {})

                    common_elements = set(user1_ratings.keys()) & set(user2_ratings.keys())

                    if common_elements:
                        user1_vector = np.array([user1_ratings[e] for e in common_elements])
                        user2_vector = np.array([user2_ratings[e] for e in common_elements])
